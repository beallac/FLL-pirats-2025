from base_robot import *

# Maze navigation script
# Sequence:
# 1. Drive 11 cm
# 2. Turn -34 degrees
# 3. Drive 36 cm
# 4. Turn 72 degrees
# 5. Drive 12.5 cm
# 6. Turn -22.5 degrees
# 7. Drive 2 cm
# 8. Turn -50 degrees

if __name__ == "__main__":
    br = BaseRobot()
    br.driveForDistance(
        distance=110, speedPct=80, then=Stop.BRAKE, waiting=True
    )  # 11 cm
    br.turnInPlace(angle=-34, speedPct=50, waiting=True)  # -38 degrees
    br.driveForDistance(
        distance=360, speedPct=80, then=Stop.BRAKE, waiting=True
    )  # 36 cm
    br.turnInPlace(angle=65, speedPct=50, waiting=True)  # 72 degrees
    br.driveForDistance(
        distance=140, speedPct=80, then=Stop.BRAKE, waiting=True
    )  # 14 cm

    br.moveLeftAttachmentMotorForDegrees(degrees=-980, speedPct=50)  # Open claw

    br.turnInPlace(angle=-22.5, speedPct=50, waiting=True)  # -31 degrees

    # Drive forward 3.00 cm (30.0 mm) and turn -16 degrees
br.driveForDistance(distance=40.0, speedPct=80, then=Stop.BRAKE, waiting=True)
br.turnInPlace(angle=-25, speedPct=50, waiting=True)
# Turn 21 degrees
br.turnInPlace(angle=21, speedPct=50, waiting=True)
# Drive 11 cm (110 mm)
br.moveLeftAttachmentMotorForDegrees(degrees=640, speedPct=50)  # Open claw
br.driveForDistance(distance=110, speedPct=80, then=Stop.BRAKE, waiting=True)
# Turn 24 degrees
br.turnInPlace(angle=24, speedPct=50, waiting=True)
br.moveLeftAttachmentMotorForDegrees(degrees=-640, speedPct=50)  # Open claw
br.driveForDistance(distance=40, speedPct=80, then=Stop.BRAKE, waiting=True)
# Turn 44 degrees
br.turnInPlace(angle=44, speedPct=20, waiting=True)
