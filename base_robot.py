...
class BaseRobot:
    ...
    def __init__(self):
        ...
        self.robot.settings(
            ...
            straight_acceleration=RescaleStraightAccel(DEFAULT_BIG_MOT_ACCEL_PCT),
            turn_rate=RescaleTurnSpeed(DEFAULT_TURN_SPEED_PCT),
            turn_acceleration=RescaleTurnAccel(DEFAULT_TURN_ACCEL_PCT),
        )

        self.frontAttachmentMotor: Motor = Motor(Port.D)
        self.rearAttachmentMotor: Motor = Motor(Port.C)
        self.frontAttachmentMotor.control.limits(acceleration=20000)
        self.rearAttachmentMotor.control.limits(acceleration=20000)
    ...
    def moveLeftAttachmentMotorForDegrees(
        ...
        self.frontAttachmentMotor.run_angle(
    ...
    def moveLeftAttachmentMotorUntilStalled(
        ...
        self.frontAttachmentMotor.run(speed)
        while abs(self.frontAttachmentMotor.load()) < load:
            wait(25)
        self.frontAttachmentMotor.hold()
    ...
    def moveRightAttachmentMotorForDegrees(
        ...
        # now the real work begins!
        speed = RescaleMedMotSpeed(speedPct)
        self.rearAttachmentMotor.run_angle(
            speed=speed, rotation_angle=degrees, wait=waiting
        )
    ...
    def moveRightAttachmentMotorUntilStalled(
        ...
        speed: int = RescaleMedMotSpeed(speedPct)
        load: int = RescaleMedMotTorque(stallPct)
        self.rearAttachmentMotor.run(speed)
        while abs(self.rearAttachmentMotor.load()) < load:
            wait(25)
        self.rearAttachmentMotor.hold()
    ...
    def driveForMillis(
        ...
        """Drives the robot for the given amount of milliseconds.
        ...
        Args:
            millis (REQUIRED integer, > 0): how many milliseconds the robot
                will drive. A millisecond is 0.001 of a second, so 5000 is 5 s.
            speedPct (OPTIONAL integer, -100..100 except 0): How fast to drive.
                Positive = forward, negative = reverse.
            gyro (OPTIONAL bool): Use the gyro. Defaults to True; set False for
                wall following.
            accelerationPct (OPTIONAL int > 0): How fast the robot accelerates.
        """
        speed = RescaleStraightSpeed(speedPct)
